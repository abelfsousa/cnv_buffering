...
protein_associations_mRNA <- readRDS("./files/protein_associations_tcga_ccle_rna_reg_genome_wide.rds")
...
signf_protein_associations_cnv_mrna <- signf_protein_associations_cnv[, c("controlling", "controlled", "beta_cnv", "fdr")] %>%
    ...
    filter(fdr_rna < 0.1) %>%
    dplyr::select(-lrt_p)
write.table(signf_protein_associations_cnv_mrna, file = "./files/signf_protein_associations_cnv_mrna_genomeWide_fdr10.txt", sep = "\t", quote = F, row.names=F)
...
write.table(as.data.frame(proteins_controlled_2more), "./files/proteins_controlled_2more_sameChr_genomeWide_fdr10.txt", row.names=F, quote=F, sep="\t")
...
write.table(as.data.frame(proteins_controlled_2more_pairsCor), "./files/proteins_controlled_2more_pairsCor_genomeWide_fdr10.txt", row.names=F, quote=F, sep="\t")
...
ggsave(filename="proteins_controlled_2more_sameChr_pairsCor_genomeWide_fdr10.png", plot=proteins_controlled_2more_pairsCor_plot, path="./plots/protein_associations_genome_wide/")
unlink("proteins_controlled_2more_sameChr_pairsCor_genomeWide_fdr10.png")





#save(list=ls(), file="../r_workspaces/protein_associations_genome_wide_controlling_genomic_proximity.RData")
