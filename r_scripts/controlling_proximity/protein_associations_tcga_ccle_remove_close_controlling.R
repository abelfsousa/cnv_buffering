...
signf_protein_associations_cnv_mrna <- read_tsv("./files/signf_protein_associations_cnv_mrna.txt")



# proteins being controlled by at least 2 proteins on the same chromosome
proteins_controlled_2more <- read_tsv("./files/proteins_controlled_2more_sameChr.txt")



# proteins being controlled by at least 2 proteins on the same chromosome
# CNV correlation by protein pairs on the same chromosome
proteins_controlled_2more_pairsCor <- read_tsv("./files/proteins_controlled_2more_pairsCor.txt")
...
signf_protein_associations_cnv_mrna <- signf_protein_associations_cnv_mrna %>%
	...
	filter(keep == 1) %>%
	dplyr::select(-keep)
write.table(signf_protein_associations_cnv_mrna, file = "./files/signf_protein_associations_cnv_mrna_filtered_close_controlling.txt", sep = "\t", quote = F, row.names=F)
...
ggsave(filename="proteins_controlled_2more_sameChr_pairsCor2.png", plot=proteins_controlled_2more_pairsCor_plot, path="./plots/protein_associations_tcga_ccle/")
unlink("proteins_controlled_2more_sameChr_pairsCor2.png")
...
save(list=ls(), file="./r_workspaces/protein_associations_tcga_ccle_remove_close_controlling.RData")
