...
write.table(cnv_rna_protein, "./files/protein_attenuation_wout_minus2.txt", sep="\t", quote=F, row.names=F)
...
comparison_att_plot <- ggplot(comparison_att) +
  ...
  labs(x = "Attenuation with -2", y = "Attenuation without -2")
ggsave(filename="comparison_att_plot.png", plot = comparison_att_plot, path = "./plots/protein_attenuation_tcga_ccle2/", width=5, height=5)
unlink("comparison_att_plot.png")
...
correlation_plot1 <- ggplot(cnv_rna_protein %>% filter(r_cnv_rna > -0.2 & r_cnv_prot > -0.2) %>% inner_join(comparison_att[, c("gene", "same_class")], by = "gene")) +
  geom_point(aes(x = r_cnv_rna, y = r_cnv_prot, color = same_class), size= 1) +
...
ggsave(filename="protein_attenuation_tcga_ccle_scatterplot.png", plot = correlation_plot1, path = "./plots/protein_attenuation_tcga_ccle2/", width=5, height=5)
...
ggsave(filename="protein_attenuation_tcga_ccle_scatterplot2.png", plot = p2, path = "./plots/protein_attenuation_tcga_ccle2/", width=5, height=5)
...
ggsave(filename="protein_attenuation_tcga_ccle_scatterplot3.png", plot = correlation_plot3, path = "./plots/protein_attenuation_tcga_ccle2/", width=5, height=5)
unlink("protein_attenuation_tcga_ccle_scatterplot3.png")




save(list=ls(), file = "./r_workspaces/protein_attenuation_tcga_ccle2.RData")
