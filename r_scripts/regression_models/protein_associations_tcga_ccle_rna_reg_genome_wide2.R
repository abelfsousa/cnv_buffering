...
linear_model <- function(py_prot, py_rna, px_rna, gend, canc, bat, prot, ag){

	## define covars matrix
	covars <- bind_cols(py_prot, py_rna, px_rna, gender = gend, cancer = canc, batch = bat, proteomics = prot, age = ag) %>%
		dplyr::rename(py_prot = prot_log2FC, py_rna = rna_log2CPM, px_rna = rna_log2CPM1)
	...
	## fit the models
	#restricted/reduced model (without rna)
	...
	return( list(c("beta_rna" = beta_rna, "lrt_p" = lrt_p)) )
...
model_data <- protein_interactions %>%
  ...
  inner_join(rna, by=c("controlling" = "gene")) %>%
  dplyr::rename(controlling_rna = rna_log2CPM) %>%
  mutate(gender = list(metadata$gender), cancer = list(metadata$cancer), batch = list(metadata$batch), proteomics = list(metadata$proteomics), age = list(metadata$age) ) %>%
	rowwise() %>%
	mutate(res = linear_model(controlled_protein, controlled_rna, controlling_rna, gender, cancer, batch, proteomics, age)) %>%
	ungroup() %>%
  ...
  #dplyr::arrange(fdr)
saveRDS(model_data, file = "./files/protein_associations_tcga_ccle_rna_reg_genome_wide.rds")
