...
write.table(common_samples, file = "./files/cptac_cellLines_samples_prot_rna_cnv.txt", sep = "\t", quote = F, row.names=F, col.names=F)
...
# select only the proteins pairs for which interactor.A %in% genomics and interactor.B %in% transcriptomics & proteomics
keep <- apply( keep_interactions, 1, function(ppair) ppair[1] %in% rownames(cnv) & ppair[2] %in% rownames(rna) & ppair[2] %in% rownames(proteomics) )
...
#examples
#linear_model("COG3", "COG2", proteomics, rna, cnv, metadata)
#linear_model("TRMT6", "TRMT61A", proteomics, rna, cnv, metadata)
...
protein_associations <- parApply( cl, keep_interactions, 1, function(x) linear_model(x[[1]], x[[2]], proteomics, rna, cnv, metadata) )
...
protein_associations <- protein_associations %>%
    ...
    dplyr::arrange(fdr)
write.table(protein_associations, file = "./files/protein_associations_tcga_ccle_cnv_reg.txt", sep = "\t", quote = F, row.names=F)
...
save(list=ls(), file="./r_workspaces/protein_associations_tcga_ccle_cnv_reg.RData")
